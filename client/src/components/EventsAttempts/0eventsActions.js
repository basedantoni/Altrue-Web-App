...
export const getEvents = (searchTerm, geoLocation) => {
    return dispatch => {
        ...
        const anonymousAccessOAuthToken = "6WBXWFIZ7CURSIDSYXLF";
        ...
        const CORSProxyServerUrl = "http://frozen-beyond-94800.herokuapp.com";
...
export const getEventsBySearchBar = (loc, searchTerm, eventDate) => {
    return (dispatch) => {
        ...
        return fetch(CORSProxyServerUrl + eventbriteUrlSearch, {
            method: "GET",
            headers: {
                    "Authorization": 'Bearer 6WBXWFIZ7CURSIDSYXLF',
                    "Origin": homePageUrl
                }
            }
        )
...
export const removeEvent = (eventData, userId) => {
    return dispatch => {
        const url = `/routes/api/users/${userId}/events/${eventData.id}` 
        fetch(url,{
            method: "DELETE",
            headers: {
                'Accept': 'application/json',
